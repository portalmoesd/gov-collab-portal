...
(async function(){
  ...
  if (!me) return;
  if (String(me.role || '').toLowerCase() !== 'super_collaborator') {
  ...
  function statusBadgeClass(status){
    ...
    if (['submitted_to_super_collaborator'].includes(s)) return 'is-review';
    if (['submitted_to_supervisor','submitted_to_chairman'].includes(s)) return 'is-submitted';
    if (['approved_by_super_collaborator','approved_by_supervisor','approved_by_chairman','approved_by_minister'].includes(s)) return 'is-approved';
  ...
  function appendSectionActions(target, section){
    ...
    const canDecision = !['approved_by_super_collaborator','submitted_to_supervisor','approved_by_supervisor','approved_by_chairman','approved_by_minister','locked'].includes(stage);
    if (canDecision) {
...
modalBackdrop.addEventListener('click', (e) => {
  ...
  submitDocBtn.addEventListener('click', async () => {
    ...
    if (!confirm('Submit all approved assigned sections to Supervisor?')) return;
    setMsg('');
    try {
      const result = await window.GCP.apiFetch('/tp/submit-approved-to-supervisor', {
        method: 'POST',
        body: JSON.stringify({ eventId: currentEventId })
      });
      if (result && Number(result.submitted || 0) > 0) setMsg('Approved sections submitted to Supervisor.');
