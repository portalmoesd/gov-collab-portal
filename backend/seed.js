/**
 * GOV COLLAB PORTAL - seed.js
 * Seeds: roles, default sections, full list of countries, and initial users (password '123' hashed).
 *
 * Run: node seed.js
 *
...
// Initial users (EDIT THIS LIST to match your real users).
// Per blueprint: initial passwords are '123' (hashed). (Blueprint v2, section 10)
const INITIAL_USERS = [
...
async function main() {
  ...
  try {
    ...
    for (const [username, fullName, email, roleKey] of INITIAL_USERS) {
      const roleRow = await client.query(`SELECT id FROM roles WHERE key=$1`, [roleKey]);
      if (!roleRow.rows[0]) throw new Error(`Role not found: ${roleKey}`);
